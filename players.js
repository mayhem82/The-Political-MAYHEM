...
Promise.all([
  ...
  fetch('data/runtime/new-south-wales-parliamentary-players.json?'+Date.now(),{cache:'no-store'}).then(r=>r.json()),
  fetch('data/runtime/political-competitions.json?'+Date.now(),{cache:'no-store'}).then(r=>r.json())
]).then(([fed,fedRosters,st,qld,wa,nsw,competitions])=>{
  ...
  const compactField=(data,label)=>{
    ...
    const players=(data.players||[]).map(r=>{
      const abbr=r[ix.party_abbreviation],m=data.party_map?.[abbr]||{};
      const chamber=ix.chamber==null?(data.chamber||'LEGISLATIVE_ASSEMBLY'):r[ix.chamber];
      const electorate=r[ix.electorate];
      return {actor_id:r[ix.actor_id],name:r[ix.name],chamber,electorate,party_abbreviation:abbr,party_id:m.party_id??null,team_name:m.short_name||m.name||'Independent',role:chamber==='LEGISLATIVE_COUNCIL'?'Member of the Legislative Council':`Member for ${electorate}`,status:'ACTIVE'};
    });
    return {id:data.competition_id,label,players,teams:(data.teams||[]).map(t=>({party_id:t.party_id,name:t.short_name||t.name,player_count:t.player_count})),independents:data.counts.independents};
  };
  ...
  const fields=[
    ...
    compactField(nsw,'NSW'),compactField(qld,'Queensland'),compactField(wa,'Western Australia'),
  ...
  const draw=()=>{
    ...
    const q=search.value.trim().toLowerCase(),tv=team.value,cv=chamber.value;
  ...
  document.getElementById('player-meta').textContent=`${fields.reduce((n,f)=>n+f.players.length,0)} parliamentary players ingested across Federal Finals and ${progress.state_territory_jurisdictions_with_complete_current_player_rosters||0} state/territory competitions · ${progress.remaining_jurisdictions?.length||0} state competitions still roster-pending · ${nsw.snapshot_id}`;
  rebuild();
