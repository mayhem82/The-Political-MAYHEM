...
Promise.all([
  ...
  const electoralBlock=(field,p)=>{
    ...
    if(e.state==='MAJOR_PARTY_DIVISION_DERIVED')return`<div class="player-section-label">ELECTORAL PERFORMANCE</div><div class="electoral-stats"><span><small>EVENT</small><b>${esc(e.event)}</b></span><span><small>PRIMARY VOTE</small><b>${esc(pct(e.primary))}</b><em>${esc(e.electionPartyCode)} at election</em></span><span><small>DIVISION 2PP · PARTY SIDE</small><b>${esc(pct(e.partySide2pp))}</b><em>Statistical 2PP, not candidate TCP</em></span><span><small>ALP 2PP</small><b>${esc(pct(e.tppAlp))}</b></span><span><small>COALITION 2PP</small><b>${esc(pct(e.tppLnc))}</b></span></div><div class="electoral-note">Division-level AEC statistics. Party-side 2PP is derived from the division result and is not represented as candidate-specific TCP.</div>`;
    return`<div class="player-section-label">ELECTORAL PERFORMANCE</div><div class="electoral-stats"><span><small>EVENT</small><b>${esc(e.event)}</b></span><span><small>ALP 2PP</small><b>${esc(pct(e.tppAlp))}</b></span><span><small>COALITION 2PP</small><b>${esc(pct(e.tppLnc))}</b></span></div><div class="electoral-note">Division context only. ${esc(e.message)}</div>`;
  };
