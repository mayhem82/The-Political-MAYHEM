...
Promise.all([
  ...
  const contestById=new Map((contestLedger.contests||[]).map(x=>[x.contest_id,x]));
  const slateByPlayer=new Map();
  for(const entry of slate.entries||[]){if(!slateByPlayer.has(entry.actor_id))slateByPlayer.set(entry.actor_id,[]);slateByPlayer.get(entry.actor_id).push(entry)}
  ...
  const senateRecords=new Map((senateElectoral.current_senator_records||[]).map(r=>[r.actor_id,r]));
  const senateUnresolved=new Map((senateElectoral.unresolved_current_senators||[]).map(r=>[r.actor_id,r]));
  ...
  const addFinal=(actorId,row)=>{if(!actorId)return;if(!finalsByPlayer.has(actorId))finalsByPlayer.set(actorId,[]);finalsByPlayer.get(actorId).push(row)};
  for(const r of electoral.candidate_specific_results||[])addFinal(r.actor_id,{event:r.event,year:eventYear(r),jurisdiction:'Federal Finals',venue:r.division||r.state||'Federal',team:r.election_party_name||r.election_party_code||'Unknown team',result:r.result_state||'WIN',source_class:r.source_class||'PRIMARY_ELECTORAL_AUTHORITY'});
  for(const r of senateElectoral.current_senator_records||[])addFinal(r.actor_id,{event:r.event,year:Number(r.election_year)||eventYear(r),jurisdiction:'Federal Finals',venue:r.state||'Senate',team:r.election_party_name||r.election_party_code||'Unknown team',result:r.result_state||'WIN',source_class:r.source_class||'PRIMARY_ELECTORAL_AUTHORITY'});
  for(const rows of finalsByPlayer.values())rows.sort((a,b)=>(b.year||0)-(a.year||0)||String(b.event).localeCompare(String(a.event)));
  ...
  const params=new URLSearchParams(location.search),requested=params.get('jurisdiction'),requestedPlayer=params.get('player');if(fields.some(f=>f.id===requested))jurisdiction.value=requested;if(requestedPlayer)search.value=requestedPlayer;

  if(registryHost)registryHost.innerHTML=`<b>STAT COVERAGE</b><span>Federal House candidate-specific electoral statistics are loaded for 150/150 current House players. Federal Senate elected-record and group-vote context is loaded for ${esc(senateElectoral.coverage?.current_senators_with_elected_record||0)}/76 current Senators; ${esc(senateElectoral.coverage?.current_senators_without_elected_record||0)} current replacements or unmatched Senators are explicitly withheld from predecessor statistics. Select FINALS HISTORY on a player card to view source-backed election appearances currently ingested.</span><small>Career finals totals remain withheld until historical election coverage is complete. Missing history is never rendered as zero. Registry ${esc(statRegistry.registry_id||'')}</small>`;
  ...
  const fieldStats=field=>{
    ...
    if(field.id==='AUS-FED')stats.push(
      ...
      {label:'SENATE ELECTED RECORDS',value:`${senateElectoral.coverage?.current_senators_with_elected_record||0}/76`},
      {label:'SENATE REPLACEMENTS / UNMATCHED',value:senateElectoral.coverage?.current_senators_without_elected_record??'—'}
    );
  ...
  const competitionSlateBlock=p=>{
    const entries=slateByPlayer.get(p.actor_id)||[];
    if(!entries.length)return`<div class="player-section-label">COMPETITION SLATE</div><div class="player-evidence muted-evidence"><small>SLATE INGESTION</small><p>No source-backed contest entry is registered for this player yet.</p><span>This is an ingestion state, not evidence that the player is not competing or will not compete in another Political MAYHEM competition.</span></div>`;
    return`<div class="player-section-label">COMPETITION SLATE</div>${entries.map(entry=>{const contest=contestById.get(entry.contest_id);return`<div class="player-evidence"><small>${esc(words(entry.competition_class))} · ${esc(words(entry.participation_role))}</small><p>${esc(contest?.name||entry.contest_id)}</p><span>${esc(words(entry.participation_state))} · ${esc(words(entry.side))} · RESULT ${esc(words(entry.result_state))}</span></div>`}).join('')}`;
  };

  const finalsHistoryBlock=(field,p)=>{
    const rows=field.id==='AUS-FED'?(finalsByPlayer.get(p.actor_id)||[]):[];
    if(!rows.length)return`<div class="player-section-label">FINALS HISTORY</div><div class="player-evidence muted-evidence"><small>HISTORICAL INGESTION</small><p>No source-backed election-final history is currently ingested for this player.</p><span>This is unknown history, not zero finals and not zero wins.</span></div><div class="finals-coverage-note">CAREER TOTAL WITHHELD · historical election coverage is incomplete.</div>`;
    const wins=rows.filter(r=>r.result==='WIN').length,losses=rows.filter(r=>r.result==='LOSS').length;
    return`<div class="player-section-label">FINALS HISTORY</div><div class="finals-summary"><span><small>KNOWN FINALS</small><b>${rows.length}</b></span><span><small>KNOWN WINS</small><b>${wins}</b></span><span><small>KNOWN LOSSES</small><b>${losses}</b></span></div><div class="finals-history-list">${rows.map(r=>`<div class="finals-history-row"><small>${esc(r.event)} · ${esc(r.jurisdiction)}</small><b>${esc(r.team)}</b><span>${esc(r.venue)} · <strong class="${r.result==='WIN'?'finals-result-win':'finals-result-loss'}">${esc(r.result)}</strong></span></div>`).join('')}</div><div class="finals-coverage-note"><b>PARTIAL CAREER HISTORY.</b> These are the finals currently supported by ingested electoral-authority records. Career finals are not totalled until historical coverage is complete; missing contests are not treated as losses.</div>`;
  };

  const federalElectoralStats=p=>{
    if(p.chamber==='SENATE'){
      const r=senateRecords.get(p.actor_id);
      if(r)return{state:'SENATE_ELECTED',event:r.event,electionParty:r.election_party_name||r.election_party_code,electedOrder:r.elected_order,groupName:r.group_name||r.group_code,groupVotes:r.group_first_preference_votes,stateFormal:r.state_formal_votes,groupPercent:r.group_first_preference_percent};
      const unresolved=senateUnresolved.get(p.actor_id);
      return{state:'SENATE_REPLACEMENT',message:unresolved?.reason||'No directly matched 2025 or 2022 elected-Senator record is loaded for this current Senator.'};
    }
  ...
  const electoralBlock=(field,p)=>{
    ...
    if(e.state==='SENATE_ELECTED')return`<div class="player-section-label">ELECTORAL PERFORMANCE</div><div class="electoral-stats"><span><small>EVENT / COHORT</small><b>${esc(e.event)}</b><em>${esc(e.electionParty||'')} at election</em></span><span><small>ELECTED ORDER</small><b>${e.electedOrder==null?'—':esc('#'+e.electedOrder+' in '+String(p.state||'').toUpperCase())}</b></span><span><small>GROUP FIRST PREFERENCE</small><b>${esc(pct(e.groupPercent))}</b><em>${e.groupVotes==null?'—':esc(Number(e.groupVotes).toLocaleString())+' votes'} · ${esc(e.groupName||'group')}</em></span><span><small>STATE FORMAL VOTES</small><b>${e.stateFormal==null?'—':esc(Number(e.stateFormal).toLocaleString())}</b></span></div><div class="electoral-note">AEC final Senate result · group first preference is team/group context, not this Senator's individual primary vote.</div>`;
    if(e.state==='SENATE_REPLACEMENT')return`<div class="player-section-label">ELECTORAL PERFORMANCE</div><div class="player-evidence muted-evidence"><small>SENATE REPLACEMENT / UNMATCHED COHORT</small><p>${esc(e.message)}</p><span>Predecessor election statistics are deliberately not inherited.</span></div>`;
  ...
  const currentPanel=(field,p,s)=>`${`<div class="player-section-label">CURRENT PERFORMANCE</div><div class="player-card-stats"><div><small>LEDGER EVENTS</small><strong>${s.events}</strong></div><div><small>VERIFIED</small><strong>${s.verified}</strong></div><div><small>SOURCES</small><strong>${s.sources}</strong></div><div><small>ROLE CHANGES</small><strong>${s.roleChanges}</strong></div><div><small>TARGET EFFECTS</small><strong>${s.targetEffects}</strong></div><div><small>CONTRADICTIONS</small><strong>${s.contradictions}</strong></div></div><div class="player-form-line"><span><small>FORM</small><b>${esc(words(s.form))}</b></span><span><small>TREND</small><b>${esc(words(s.trend))}</b></span></div>${electoralBlock(field,p)}${competitionSlateBlock(p)}<div class="player-section-label">STRUCTURAL</div><div class="player-structural"><span><small>CURRENT ROSTER FIELD</small><b>${esc(field.label)}</b></span><span><small>TEAM</small><b>${esc(p.team_name)}</b></span><span><small>CHAMBER</small><b>${esc(chamberLabel(p.chamber))}</b></span><span><small>ELECTORATE / STATE</small><b>${esc(placeLabel(p)||'—')}</b></span></div><div class="electoral-note">Political MAYHEM players can compete across multiple political competitions. This field shows the parliamentary roster currently in view; it is not the player's complete competition slate.</div>${s.last?`<div class="player-evidence"><small>LATEST PLAYER EVIDENCE · ${esc(s.last.event_date||s.last.captured_at||'')}</small><p>${esc(s.last.claim||'')}</p><span>${esc(words(s.last.evidence_state||'UNKNOWN'))} · ${esc(words(s.last.source_class||'UNKNOWN'))}</span></div>`:`<div class="player-evidence muted-evidence"><small>PLAYER EVIDENCE</small><p>No actor-specific forward intelligence event is registered yet. This is an absence from the current ledger, not a neutral performance rating.</p></div>`}`}`;
  ...
  const draw=()=>{
    ...
    host.innerHTML=visible.map(p=>{const s=playerStats(field,p),electoralState=field.id==='AUS-FED'?federalElectoralStats(p):null,hasElectoralData=electoralState&&['SENATE_ELECTED','CANDIDATE_SPECIFIC','MAJOR_PARTY_DIVISION_DERIVED','DIVISION_CONTEXT_ONLY'].includes(electoralState.state),intelState=s.events?'INTELLIGENCE ACTIVE':hasElectoralData?'ELECTORAL DATA LOADED':'ROSTER ONLY';return`<article class="player-card"><div class="player-card-head"><div><span class="player-label">PLAYER · ${esc(chamberLabel(p.chamber))}</span><h3>${esc(p.name)}</h3><p>${esc(p.role||'')}</p></div><div class="directory-meta"><strong>${esc(p.team_name)}</strong><span>${esc(placeLabel(p))}</span><span class="player-state-pill">${esc(intelState)}</span></div></div><div class="player-view-tabs" role="group" aria-label="${esc(p.name)} player history selector"><button type="button" class="player-view-tab" data-player-view="current" aria-pressed="true">CURRENT</button><button type="button" class="player-view-tab" data-player-view="finals" aria-pressed="false">FINALS HISTORY</button></div><div data-player-panel="current">${currentPanel(field,p,s)}</div><div data-player-panel="finals" hidden>${finalsHistoryBlock(field,p)}</div></article>`}).join('')||'<div class="empty-state">No players match these filters.</div>';
    const next=new URLSearchParams();next.set('jurisdiction',field.id);if(search.value.trim())next.set('player',search.value.trim());history.replaceState(null,'',`${location.pathname}?${next.toString()}`)
  };

  host.addEventListener('click',event=>{
    const button=event.target.closest('.player-view-tab');if(!button)return;
    const card=button.closest('.player-card');if(!card)return;
    const view=button.dataset.playerView;
    card.querySelectorAll('.player-view-tab').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));
    card.querySelectorAll('[data-player-panel]').forEach(panel=>{panel.hidden=panel.dataset.playerPanel!==view});
  });
  ...
  const progress=competitions.ingestion_progress||{};document.getElementById('player-meta').textContent=`${fields.reduce((n,f)=>n+f.players.length,0)} parliamentary players ingested across Federal Finals and all ${progress.state_territory_jurisdictions_with_complete_current_player_rosters||8} state/territory competitions · House candidate-specific stats ${electoral.coverage?.current_players_with_any_candidate_specific_stats||0}/150 · Senate elected-record stats ${senateElectoral.coverage?.current_senators_with_elected_record||0}/76 · finals history is selectable per player and expands only from source-backed election records.`;
  rebuild();
